...
CIMGUI_API void ImFont_CalcTextSizeA(const struct ImFont* font, ImVec2* pOut, float size, float max_width, float wrap_width, ImStr text, const char** remaining)
{
    *pOut = font->CalcTextSizeA(size, max_width, wrap_width, text, remaining);
}

CIMGUI_API const char* ImFont_CalcWordWrapPositionA(const struct ImFont* font, float scale, ImStr text, float wrap_width)
{
    return font->CalcWordWrapPositionA(scale, text, wrap_width);
}
...
CIMGUI_API void ImFont_RenderText(const struct ImFont* font, ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4* clip_rect, ImStr text, float wrap_width, bool cpu_fine_clip)
{
    return font->RenderText(draw_list, size, pos, col, *clip_rect, text, wrap_width, cpu_fine_clip);
}
